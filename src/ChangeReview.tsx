...
export function ChangeReview({ project }: { project: Snapshot }) {
  ...
  return <>
    ...
    <div className="modal-footer">
      ...
      <button className="primary" disabled={busy || !editable || !review.data?.changes.length || review.isFetching || !!review.error || !!project.changesError} onClick={() => ui.set({ dialog: "commit" })}>Commit へ</button>
    </div>
  </>;
}
...
function ReviewContent({ data, initialMaster, disabled }: { data: ChangeReviewData; initialMaster: string | null; disabled: boolean }) {
  ...
  const masters = [...new Set(data.changes.map(c => c.masterId))];
  const masterId = masters.find(id => id === selectedMaster) ?? masters[0];
  if (!masterId) return <div className="review-message empty-state">管理対象の変更はありません。</div>;
  return <div className="review-layout">
    <nav className="review-masters" aria-label="変更した Master">
      ...
      {masters.map(id => <button key={id} className={id === masterId ? "active" : ""} aria-current={id === masterId ? "page" : undefined} onClick={() => setSelectedMaster(id)}>
        ...
        <small>{data.changes.filter(c => c.masterId === id).length} 件の変更</small>
      </button>)}
    </nav>
  ...
  return <div className="editor review-editor">
    ...
    <div className="editor-content review-content">
        ...
        <div className="ag-container">
          <AgGridReact<ReviewRow>
            ...
            onCellFocused={e => {
              ...
              if (row && id.startsWith("data:")) setSelection({ rowId: row.id, column: id.slice(5) });
            }}
            onColumnHeaderClicked={e => {
              if (!("getColId" in e.column)) return;
              const id = e.column.getColId();
              if (id.startsWith("data:")) setSelection({ rowId: null, column: id.slice(5) });
            }}
      ...
      <aside className={`review-inspector${table ? "" : " review-settings"}`} aria-label="変更の詳細">
        <div className="inspector-heading"><h2>変更の詳細</h2>{table && <button onClick={() => setSelection(null)}>テーブル・設定</button>}</div>
        <div className="review-details-body">
          ...
          {relevant.map(change => <ChangeDetail key={JSON.stringify(change)} change={change} disabled={disabled || data.before === null} />)}
